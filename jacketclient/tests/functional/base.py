...
def pick_network(networks):
    network_name = os.environ.get('OS_jacketclient_NETWORK')
    if network_name:
...
class ClientTestBase(testtools.TestCase):
    """Base test class for read only python-jacketclient commands.

    This is a first pass at a simple read only python-jacketclient test. This
    ...
    * initially just check return codes, and later test command outputs
    ...
    def setUp(self):
        ...
        self.client = self._get_jacketclient(session)
        ...
        if "network" not in CACHE:
            ...
            proxy_api_version = jacketclient.api_versions.APIVersion('2.35')
    ...
    def _get_jacketclient(self, session):
        nc = jacketclient.client.Client("2", session=session)
            ...
            if "min_api_version" not in CACHE:
                ...
                if not hasattr(v, 'version') or not v.version:
                    ...
                    CACHE["min_api_version"] = (
                        jacketclient.api_versions.APIVersion("2.0"))
                    CACHE["max_api_version"] = (
                        jacketclient.api_versions.APIVersion("2.0"))
                else:
                    CACHE["min_api_version"] = (
                        jacketclient.api_versions.APIVersion(v.min_version))
                    CACHE["max_api_version"] = (
                        jacketclient.api_versions.APIVersion(v.version))

            if self.COMPUTE_API_VERSION == "2.latest":
                requested_version = min(jacketclient.API_MAX_VERSION,
                                        CACHE["max_api_version"])
            else:
                requested_version = jacketclient.api_versions.APIVersion(
    ...
    def jacket(self, action, flags='', params='', fail_ok=False,
             endpoint_type='publicURL', merge_stderr=False):
        if self.COMPUTE_API_VERSION:
            flags += " --os-jacket-api-version %s " % self.COMPUTE_API_VERSION
        return self.cli_clients.jacket(action, flags, params, fail_ok,
    ...
    def wait_for_server_os_boot(self, server_id, timeout=300,
        ...
        console = None
        while time.time() - start_time < timeout:
            console = self.jacket('console-log %s ' % server_id)
    ...
    def _create_server(self, name=None, with_network=True, add_cleanup=True,
        ...
        jacketclient.v1.shell._poll_for_status(
    ...
    def skip_if_neutron(self):
        if CACHE["use_neutron"]:
            self.skipTest('jacket-network is not available')
...
class TenantTestBase(ClientTestBase):
    ...
    def another_jacket(self, action, flags='', params='', fail_ok=False,
                     endpoint_type='publicURL', merge_stderr=False):
        flags += " --os-jacket-api-version %s " % self.COMPUTE_API_VERSION
        return self.cli_clients_2.jacket(action, flags, params, fail_ok,
                                       endpoint_type, merge_stderr)
