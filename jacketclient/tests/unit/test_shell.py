...
import jacketclient.shell
from jacketclient.tests.unit import fake_actions_module
from jacketclient.tests.unit import utils

FAKE_ENV = {'OS_USERNAME': 'username',
            'OS_PASSWORD': 'password',
            'OS_TENANT_NAME': 'tenant_name',
            'OS_AUTH_URL': 'http://no.where/v1.0',
...
FAKE_ENV3 = {'OS_USER_ID': 'user_id',
             ...
             'OS_TENANT_ID': 'tenant_id',
             'OS_AUTH_URL': 'http://no.where/v1.0',
             'JACKET_ENDPOINT_TYPE': 'jacketURL',
...
class DeprecatedActionTest(utils.TestCase):
    ...
    def test_init_emptyhelp_nouse(self, mock_init):
        result = jacketclient.shell.DeprecatedAction(
    ...
    @mock.patch.object(jacketclient.shell.argparse.Action, '__init__',
                       return_value=None)
    ...
    def test_get_action_nolookup(self):
        ...
        obj = jacketclient.shell.DeprecatedAction(
    ...
    @mock.patch.object(sys, 'stderr', six.StringIO())
    @mock.patch.object(jacketclient.shell.DeprecatedAction, '_get_action')
...
class ParserTest(utils.TestCase):
    ...
    def setUp(self):
        ...
        self.parser = jacketclient.shell.NovaClientArgumentParser()
...
class ShellTest(utils.TestCase):
    ...
    def setUp(self):
        ...
        self.useFixture(fixtures.MonkeyPatch(
                        'jacketclient.client.Client',
                        mock.MagicMock()))
        self.nc_util = mock.patch('jacketclient.utils.isunauthenticated').start()
        self.nc_util.return_value = False
        self.mock_server_version_range = mock.patch(
            'jacketclient.api_versions._get_server_version_range').start()
        self.mock_server_version_range.return_value = (
            jacketclient.API_MIN_VERSION,
            jacketclient.API_MIN_VERSION)
        self.orig_max_ver = jacketclient.API_MAX_VERSION
        self.orig_min_ver = jacketclient.API_MIN_VERSION
        self.addCleanup(self._clear_fake_version)
        self.addCleanup(mock.patch.stopall)

    def _clear_fake_version(self):
        jacketclient.API_MAX_VERSION = self.orig_max_ver
        jacketclient.API_MIN_VERSION = self.orig_min_ver
    ...
    def shell(self, argstr, exitcodes=(0,)):
        ...
        try:
            ...
            _shell = jacketclient.shell.OpenStackComputeShell()
    ...
    def _test_help(self, command, required=None):
        if required is None:
            required = [
                ...
                '.*?^See "jacket help COMMAND" for help on a specific command',
            ]
    ...
    @mock.patch('jacketclient.client.Client')
    @requests_mock.Mocker()
    def test_jacket_endpoint_type(self, mock_client, m_requests):
        self.make_env(fake_env=FAKE_ENV3)
        self.register_keystone_discovery_fixture(m_requests)
        self.shell('list')
        client_kwargs = mock_client.call_args_list[0][1]
        self.assertEqual(client_kwargs['endpoint_type'], 'jacketURL')
    ...
    def test_os_endpoint_type(self, mock_client, m_requests):
        self.make_env(exclude='JACKET_ENDPOINT_TYPE', fake_env=FAKE_ENV3)
        self.register_keystone_discovery_fixture(m_requests)
        self.shell('list')
        client_kwargs = mock_client.call_args_list[0][1]
    ...
    def _test_service_type(self, version, service_type, mock_client):
        if version is None:
            ...
            cmd = ('--service-type %s --os-jacket-api-version %s list' %
    ...
    def test_default_service_type(self, mock_client):
        self._test_service_type(None, 'jacket', mock_client)

    @mock.patch('jacketclient.client.Client')
    def test_v1_1_service_type(self, mock_client):
        self._test_service_type('1.1', 'jacket', mock_client)

    @mock.patch('jacketclient.client.Client')
    def test_v2_service_type(self, mock_client):
        self._test_service_type('2', 'jacket', mock_client)

    @mock.patch('jacketclient.client.Client')
    def test_v_unknown_service_type(self, mock_client):
        self.assertRaises(exceptions.UnsupportedVersion,
                          self._test_service_type,
                          'unknown', 'jacket', mock_client)

    @mock.patch('sys.argv', ['jacket'])
    @mock.patch('sys.stdout', six.StringIO())
    @mock.patch('sys.stderr', six.StringIO())
    def test_main_noargs(self):
        # Ensure that main works with no command-line arguments
        try:
            jacketclient.shell.main()
        except SystemExit:
    ...
    @mock.patch.object(jacketclient.shell.OpenStackComputeShell, 'main')
    def test_main_keyboard_interrupt(self, mock_jacket_shell):
        # Ensure that exit code is 130 for KeyboardInterrupt
        mock_jacket_shell.side_effect = KeyboardInterrupt()
        try:
            jacketclient.shell.main()
        except SystemExit as ex:
            self.assertEqual(ex.code, 130)

    @mock.patch.object(jacketclient.shell.OpenStackComputeShell, 'times')
    @requests_mock.Mocker()
    ...
    @mock.patch('jacketclient.shell.SecretsHelper.tenant_id',
                return_value=True)
    @mock.patch('jacketclient.shell.SecretsHelper.auth_token',
                return_value=True)
    @mock.patch('jacketclient.shell.SecretsHelper.management_url',
                return_value=True)
    @mock.patch('jacketclient.client.Client')
    @requests_mock.Mocker()
    def test_keyring_saver_helper(self, mock_client,
        ...
        self.assertIsInstance(keyring_saver, jacketclient.shell.SecretsHelper)
    ...
    def test_microversion_with_specified_version(self, mock_client):
        ...
        self.shell('--os-jacket-api-version 2.99 list')
        client_args = mock_client.call_args_list[1][0]
    ...
    def test_microversion_with_specified_version_out_of_range(self,
        ...
        self.assertRaises(exceptions.CommandError,
                          self.shell, '--os-jacket-api-version 2.199 list')
    ...
    @mock.patch.object(jacketclient.shell.OpenStackComputeShell, 'main')
    def test_main_error_handling(self, mock_jacket_shell):
        class MyException(Exception):
            pass
        with mock.patch('sys.stderr', six.StringIO()):
            mock_jacket_shell.side_effect = MyException('message')
            self.assertRaises(SystemExit, jacketclient.shell.main)
...
class TestLoadVersionedActions(utils.TestCase):
    ...
    def test_load_versioned_actions_with_help_on_latest(self):
        ...
        expected_desc = (" (Supported by API versions '%(start)s' - "
            ...
            'hint': jacketclient.shell.HINT_HELP_MSG}
        self.assertEqual(expected_desc,
                         shell.subcommands['another-fake-action'].description)

    @mock.patch.object(jacketclient.shell.NovaClientArgumentParser,
                       'add_argument')
