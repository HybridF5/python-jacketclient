...
from jacketclient.tests.unit.v2 import fakes
from jacketclient.v1 import flavors
...
def create_response_obj_with_jacket_header():
    resp = Response()
    resp.headers['x-jacket-request-id'] = fakes.FAKE_REQUEST_ID
    return resp
...
class BaseTest(utils.TestCase):
    ...
    def test_resource_object_with_jacket_request_ids(self):
        resp_obj = create_response_obj_with_jacket_header()
        r = base.Resource(None, {"name": "1"}, resp=resp_obj)
        self.assertEqual(fakes.FAKE_REQUEST_ID_LIST, r.request_ids)
