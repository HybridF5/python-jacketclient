...
from jacketclient import utils as nutils
...
class CheckHeadersTestCase(utils.TestCase):
    def setUp(self):
        ...
        mock_log_patch = mock.patch("jacketclient.api_versions.LOG")
...
class GetAPIVersionTestCase(utils.TestCase):
    ...
    @mock.patch("jacketclient.api_versions.APIVersion")
...
class WrapsTestCase(utils.TestCase):
    ...
    @mock.patch("jacketclient.api_versions._get_function_name")
    @mock.patch("jacketclient.api_versions.VersionedMethod")
    ...
    def test_generate_function_name(self):
        expected_name = "jacketclient.tests.unit.test_api_versions.fake_func"
...
class DiscoverVersionTestCase(utils.TestCase):
    def setUp(self):
        ...
        self.orig_max = jacketclient.API_MAX_VERSION
        self.orig_min = jacketclient.API_MIN_VERSION
        self.addCleanup(self._clear_fake_version)

    def _clear_fake_version(self):
        jacketclient.API_MAX_VERSION = self.orig_max
        jacketclient.API_MIN_VERSION = self.orig_min
    ...
    def test_server_is_too_old(self):
        ...
        fake_client.versions.get_current.return_value = mock.MagicMock(
            version="2.7", min_version="2.4")
        jacketclient.API_MAX_VERSION = api_versions.APIVersion("2.10")
        jacketclient.API_MIN_VERSION = api_versions.APIVersion("2.9")
...
class DecoratedAfterTestCase(utils.TestCase):
    def test_decorated_after(self):
        ...
        with mock.patch('warnings.warn') as mock_warn:
            Fake().foo()
            msg = ('The jacketclient.tests.unit.test_api_versions module '
