...
def check_major_version(api_version):
    ...
    :raises jacketclient.exceptions.UnsupportedVersion: if major part is not
                                                      supported
    """
        ...
        raise exceptions.UnsupportedVersion(
            _("Server version is too old. The client valid version range is "
              ...
              "range is '%(server_min)s' to '%(server_max)s'.") % {
                  'client_min': jacketclient.API_MIN_VERSION.get_string(),
                  'client_max': jacketclient.API_MAX_VERSION.get_string(),
                  'server_min': server_start_version.get_string(),
                  'server_max': server_end_version.get_string()})
    elif jacketclient.API_MAX_VERSION < server_start_version:
        raise exceptions.UnsupportedVersion(
            _("Server version is too new. The client valid version range is "
              ...
              "range is '%(server_min)s' to '%(server_max)s'.") % {
                  ...
                  'server_min': server_start_version.get_string(),
                  'server_max': server_end_version.get_string()})
    elif jacketclient.API_MAX_VERSION <= server_end_version:
        return jacketclient.API_MAX_VERSION
    elif server_end_version < jacketclient.API_MAX_VERSION:
