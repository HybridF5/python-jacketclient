...
from jacketclient import extension as ext
from jacketclient.i18n import _, _LW
...
def _log_request_id(logger, resp, service_name):
    request_id = (resp.headers.get('x-openstack-request-id') or
                  resp.headers.get('x-jacket-request-id'))
    if request_id:
...
class SessionClient(adapter.LegacyJsonAdapter):
    ...
    def request(self, url, method, **kwargs):
        ...
        # keystoneauth1, where we need to raise the jacketclient errors.
        ...
        #   check only jacket-related calls
...
class HTTPClient(object):
    USER_AGENT = 'python-jacketclient'
    ...
    def _cs_request(self, url, method, **kwargs):
        ...
        if url is None:
            ...
            # a jacket endpoint directly without "v2/<tenant-id>".
    ...
    def authenticate(self):
        ...
        if self.version == "v1.0":  # FIXME(chris): This should be better.
            while auth_url:
            ...
            # In some configurations jacket makes redirection to
            # v2.0 keystone endpoint. Also, new location does not contain
            # real endpoint, only hostname and port.
            except exceptions.AuthorizationFailure:
                if auth_url.find('v1.0') < 0:
                    auth_url = auth_url + '/v1.0'
    ...
    def _v2_auth(self, url):
        """Authenticate against a v1.0 auth service."""
        if self.auth_token:
            body = {"auth": {
...
def _construct_http_client(username=None, password=None, project_id=None,
                           ...
                           extensions=None, service_type='jacket',
                           service_name=None, volume_service_name=None,
                           ...
                           auth=None, user_agent='python-jacketclient',
...
def discover_extensions(version, only_contrib=False):
    ...
    contrib path and by entry-point 'jacketclient.extension'.

    :param version: api version
    :type version: str or jacketclient.api_versions.APIVersion
...
def _discover_via_python_path():
    for (module_loader, name, _ispkg) in pkgutil.iter_modules():
        if name.endswith('_python_jacketclient_ext'):
...
def _discover_via_entry_points():
    for ep in pkg_resources.iter_entry_points('jacketclient.extension'):
        name = ep.name
        module = ep.load()
...
def Client(version, username=None, api_key=None, project_id=None,
    ...
    credentials::
        ...
        >>> jacket = client.Client(VERSION, USERNAME, PASSWORD,
        ...                      PROJECT_ID, AUTH_URL)

    Here ``VERSION`` can be a string or
    ``jacketclient.api_versions.APIVersion`` obj. If you prefer string value,
    ...
    session API. See "The jacketclient Python API" page at
    python-jacketclient's doc.
    """
    if args:
