...
class Resource(RequestIdMixin):
    ...
    def get(self):
        ...
        Some clients, such as jacketclient have the option to lazy load the
...
class Manager(HookableMixin):
    ...
    def completion_cache(self, cache_type, obj_class, mode):
        ...
        with self.cache_lock:
            base_dir = utils.env('JACKETCLIENT_UUID_CACHE_DIR',
                                 default="~/.jacketclient")

            # NOTE(sirp): Keep separate UUID caches for each username +
            # endpoint pair
            username = utils.env('OS_USERNAME', 'JACKET_USERNAME')
            url = utils.env('OS_URL', 'JACKET_URL')
