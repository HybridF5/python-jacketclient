...
import jacketclient.auth_plugin
from jacketclient import client
from jacketclient import exceptions as exc
import jacketclient.extension
from jacketclient.i18n import _
from jacketclient import utils

DEFAULT_MAJOR_OS_JACKET_API_VERSION = "1.0"
# The default behaviour of jacket client CLI is that CLI negotiates with server
...
HINT_HELP_MSG = (" [hint: use '--os-jacket-api-version' flag to show help "
...
class SecretsHelper(object):
    ...
    def save(self, auth_token, management_url, tenant_id):
        ...
        value = "|".join([str(auth_token),
                          str(management_url),
                          str(tenant_id)])
        keyring.set_password("jacketclient_auth", self._make_key(), value)
    ...
    def management_url(self):
        ...
        management_url = None
        try:
            block = keyring.get_password('jacketclient_auth', self._make_key())
            if block:
...
class JacketClientArgumentParser(argparse.ArgumentParser):

    def __init__(self, *args, **kwargs):
        super(JacketClientArgumentParser, self).__init__(*args, **kwargs)
...
class OpenStackComputeShell(object):
    ...
    def _append_global_identity_args(self, parser, argv):
        ...
        parser.set_defaults(insecure=strutils.bool_from_string(
            utils.env('jacketclient_INSECURE', default=False)))
        parser.set_defaults(os_auth_url=utils.env('OS_AUTH_URL', 'JACKET_URL'))
        ...
        parser.set_defaults(os_project_name=utils.env(
            'OS_PROJECT_NAME', 'OS_TENANT_NAME', 'JACKET_PROJECT_ID'))
        parser.set_defaults(os_project_id=utils.env(
            'OS_PROJECT_ID', 'OS_TENANT_ID'))

    def get_base_parser(self, argv):
        parser = JacketClientArgumentParser(
            prog='jacket',
            description=__doc__.strip(),
            epilog='See "jacket help COMMAND" '
        ...
        parser.add_argument('--version',
                            action='version',
                            version=jacketclient.__version__)
        ...
        parser.add_argument(
            '--os-region-name',
            metavar='<region-name>',
            default=utils.env('OS_REGION_NAME', 'JACKET_REGION_NAME'),
        ...
        parser.add_argument(
            ...
            help=_('Defaults to jacket for most actions.'))

        parser.add_argument(
            '--service-name',
            metavar='<service-name>',
            default=utils.env('JACKET_SERVICE_NAME'),
            help=_('Defaults to env[JACKET_SERVICE_NAME].'))

        parser.add_argument(
            '--volume-service-name',
            action=DeprecatedAction,
            metavar='<volume-service-name>',
            default=utils.env('JACKET_VOLUME_SERVICE_NAME'),
        ...
        parser.add_argument(
            ...
            dest='endpoint_type',
            default=utils.env(
                'JACKET_ENDPOINT_TYPE', default=utils.env(
                    'OS_ENDPOINT_TYPE',
                    default=DEFAULT_JACKET_ENDPOINT_TYPE)),
            help=_('Defaults to env[JACKET_ENDPOINT_TYPE], '
                   'env[OS_ENDPOINT_TYPE] or ') +
                 DEFAULT_JACKET_ENDPOINT_TYPE + '.')

        parser.add_argument(
            '--os-jacket-api-version',
            metavar='<jacket-api-ver>',
            default=utils.env('OS_COMPUTE_API_VERSION',
                              default=DEFAULT_OS_JACKET_API_VERSION),
        ...
        parser.add_argument(
            ...
            metavar='<bypass-url>',
            dest='bypass_url',
            default=utils.env('jacketclient_BYPASS_URL'),
            help=_("Use this API endpoint instead of the Service Catalog. "
                   "Defaults to env[jacketclient_BYPASS_URL]."))

        # The auth-system-plugins might require some extra options
        jacketclient.auth_plugin.load_auth_system_opts(parser)
    ...
    def get_subcommand_parser(self, version, do_help=False, argv=None):
        ...
        actions_module = importutils.import_module(
            "jacketclient.v%s.shell" % version.ver_major)
    ...
    def do_bash_completion(self, _args):
        ...
        jacket.bash_completion script doesn't have to hard code them.
        """
        commands = set()
        options = set()
...
def main():
    try:
        ...
        print(_("... terminating jacket client"), file=sys.stderr)
        sys.exit(130)
